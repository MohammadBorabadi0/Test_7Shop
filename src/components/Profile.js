            ...
            <Layout>
                <section className='px-6 xl:px-3'>
                    <div className='flex gap-4 items-center mb-5'>
                        <img src={myUser.picture} alt={`${myUser.name} pic`} className='rounded-full w-16' />
                        <h1 className='text-xl font-bold'>Hello, {myUser.given_name}</h1>
                    </div>
                    <div className='flex flex-col gap-1 font-semibold'>
                        <h4>Name : <span className='text-slate-700'>{myUser.name}</span></h4>
                        <p>Email : <span className='text-slate-700'>{myUser.email}</span></p>
                    </div>
                </section>
            </Layout>
        );
