...
import { UPDATE_SORT, UPDATE_FILTERS, CLEAR_FILTERS, ADD_APPLIED_FILTERS, REMOVE_APPLIED_FILTERS } from '../actions';
...
const Modal = ({ showFilterMobile, setShowFilterMobile }) => {
    ...
    const { sort, filters, all_products, dispatch, appliedFilters } = useFilter();
    ...
    if (showFilterMobile)
        return (
            <section className={`flex lg:hidden flex-col gap-8 bg-white z-20 fixed inset-0 rounded-t-2xl px-4 py-6 animate-wiggle overflow-y-auto`}>
